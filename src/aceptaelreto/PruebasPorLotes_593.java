...
public class PruebasPorLotes_593 {
    ...
    private static int numeroPruebas(String muestras, int desde, int hasta) {
        if (desde == hasta - 1 || !hayPositivo(muestras, desde, hasta)) {
            ...
            if ((hasta - desde) % 2 == 0) {
                return 1 + numeroPruebas(muestras, desde, suma / 2)
                        + numeroPruebas(muestras, suma / 2, hasta);
            } else {
                return 1 + numeroPruebas(muestras, desde, suma / 2 + 1)
                        + numeroPruebas(muestras, suma / 2 + 1, hasta);
            }
        }
    }
